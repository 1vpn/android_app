...
fun getAvailableLocations(): List<Location> {
    ...
    return if (userData != null && userData.isPremium && userData.locations.isNotEmpty()) {
        userData.locations.filter { !it.servers.isNullOrEmpty() }
    } else {
        freeLocations
    }
}
...
fun fetchUserData(context: Context, onComplete: (() -> Unit)? = null) {
    ...
    if (context is LifecycleOwner) {
        context.lifecycleScope.launch {
            try {
                val headers = if (sessionAuthToken != null)
                    mapOf("Authorization" to "Token $sessionAuthToken")
                else
                    emptyMap()

                val response = RetrofitClient.callWithFallback { it.fetchUserData(headers) }

                if (response.isSuccessful && sessionAuthToken != null) {
                    response.body()?.let { MmkvManager.setUserData(it) }
